...
| 档位 | 需要具备的内容 |
|---|---|
| **A: Entrypoint-Only** | `AGENTS.md` + 至少一个层级入口文件 |
| **B: Routed Automation** | Profile A + 顶层存在层级的确定性引导发现机制 + 对实际存在边界的路由目录 |
| **C: Governed Enterprise** | Profile B + 治理注册表、解决方案范围清单以及治理状态工件 |
...
Profile B 和 C 的实现必须至少提供一种确定性的引导发现机制，用于解析当前组织中存在的最高层级：
...
1. 阅读[完整提案](enterprise_repo_convention.md)。
2. 选择适合你组织的一致性档位。
3. 从 [templates/](templates/) 复制相关起始文件到你的仓库中。
4. 如果你采用的是 Profile B 或 C，请定义引导发现机制（例如 `ENTERPRISE_REPO_URL` 环境变量、启动参数，或类似 `https://config.example.com/enterprise-catalog` 的固定发现端点）。
...
enterprise_repo_convention.md                      -- 提案正文
templates/
  README.md                                        -- 模板使用说明
...
提交 issue 时，请注明你的反馈对应哪一层（A 或 B）或哪一个一致性档位（A/B/C）。提交 pull request 时，请尽量将变更聚焦在提案的单一章节。
